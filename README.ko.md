...
2. `LocateBarFolia-1.0.0.jar`를 서버의 `plugins` 폴더에 넣습니다.
3. 서버를 재시작합니다.
...
| 명령어 | 권한 | 설명 |
| --- | --- | --- |
| `/locatebar` | `locatebar.use` | 자신의 LocateBar 참여 상태를 전환합니다. |
| `/locatebar on` | `locatebar.use` | 자신의 참여 상태를 켭니다. |
| `/locatebar off` | `locatebar.use` | 자신의 참여 상태를 끕니다. |
| `/locatebar toggle` | `locatebar.use` | 자신의 참여 상태를 전환합니다. |
...
| 권한 | 기본값 | 설명 |
| --- | --- | --- |
| `locatebar.use` | `true` | 플레이어가 참여 상태 명령어를 사용할 수 있게 합니다. |
...
| 옵션 | 설명 |
| --- | --- |
| `enabled-by-default` | 저장된 개인 설정이 없을 때 기본적으로 참여 상태를 켤지 정합니다. |
